...
class EventLoopManager(QObject):
    ...
    def initialize(self):
        ...
        if self._loop_initialized and self._main_loop:
    ...
    def _keep_loop_alive(self):
        ...
        try:
            ...
            # Add task to the loop - use call_soon_threadsafe for reliability
            if hasattr(self._main_loop, 'call_soon_threadsafe'):
                self._main_loop.call_soon_threadsafe(
                    lambda: self._main_loop.create_task(_dummy())
                )
        except Exception as e:
    ...
    def _check_loop_health(self):
        ...
        try:
            ...
            if not is_running and self._loop_initialized:
    ...
    def run_coroutine(self, coro, callback=None, error_callback=None, timeout=None):
        ...
        try:
            ...
            if platform.system() == "Windows":
                def create_task():
                    return loop.create_task(_safe_wrapper())
                ...
                if hasattr(loop, 'call_soon_threadsafe'):
                    ...
                    loop.call_soon_threadsafe(lambda: set_task(create_task()))
                    
                    # Give a small time for the task to be created
                    time.sleep(0.01)
                    task = task_future.result() if task_future.done() else create_task()
                else:
                    task = create_task()
            else:
