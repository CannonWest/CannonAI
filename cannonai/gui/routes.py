...
def send_message_api_route():
    ...
    print(f"[Routes] Send request: message='{message_content[:50]}...', attachments={len(attachments) if attachments else 0}")
...
def stream_message_api_route():
    ...
    if not _api_handlers or not _chat_client or not _event_loop:
        ...
        def error_stream_not_ready():
            yield f"data: {jsonify({'error': 'Server not ready for streaming.'}).get_data(as_text=True)}\n\n"
    ...
    print(f"[Routes] Stream request: message='{message_content[:50]}...', attachments={len(attachments) if attachments else 0}")
    ...
    if not message_content and not attachments:
        ...
        def error_no_content():
            yield f"data: {jsonify({'error': 'No message or attachments provided for streaming.'}).get_data(as_text=True)}\n\n"
...
def get_settings_api_route():
    ...
    if not _api_handlers or not _chat_client or not _chat_client.provider:
        ...
        logger.warning("/api/settings GET: API handlers or client not fully ready, returning defaults from main_config.")
